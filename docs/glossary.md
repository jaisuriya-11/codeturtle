...
| **norms**               | The review rules, resolved in layers: built-in defaults → global norms & packs → the target repo's `.codeturtle.yml` (project wins). See [Configuration](./configuration.md#custom-norms-global--packs--plugins).                                                |
| **pack**                | A named, reusable declarative norm set (`~/.codeturtle/norms/<name>.yml`). Activated globally via `norms.use` or per-repo via `extends:` (by name only). See [Custom norms](./configuration.md#packs--reusable-declarative-rule-sets).                            |
| **transform**           | A code plugin (`~/.codeturtle/norms/<name>.mjs`) that adjusts merged norms programmatically. Runs only when the **global** config activates it — never on a repo's say-so. See [Custom norms](./configuration.md#transforms--code-plugins-power-users).           |
